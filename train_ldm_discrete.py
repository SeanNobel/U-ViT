...
def train(config):
    ...
    def get_data_generator():
        while True:
            for data in tqdm(
                train_dataset_loader, disable=not accelerator.is_main_process, desc="epoch"
            ):
                yield data
    ...
    def dpm_solver_sample(_n_samples, _sample_steps, **kwargs):
        ...
        _z = dpm_solver.sample(_z_init, steps=_sample_steps, eps=1.0 / _schedule.N, T=1.0)
        return decode(_z)
    ...
    while train_state.step < config.train.n_steps:
        ...
        if accelerator.is_main_process and train_state.step % config.train.eval_interval == 0:
            ...
            if config.train.mode == "uncond":
                ...
                y = einops.repeat(
                    torch.arange(5, device=device) % dataset.K, "nrow -> (nrow ncol)", ncol=10
                )
